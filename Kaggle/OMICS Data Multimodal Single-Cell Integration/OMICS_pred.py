...
#tokenizer = SMILESAtomTokenizer(smiles=list(df.molecule_smiles))
#print(tokenizer.vocabulary)
def Toeknit(mol):
    tokenizer = SMILESAtomTokenizer(smiles=list(mol))
    return tokenizer(mol)


df['Tok_mol'] = df['molecule_smiles'].apply(Toeknit)
...
df['block1'] = df['buildingblock1_smiles'].apply(Toeknit)
df['block2'] = df['buildingblock2_smiles'].apply(Toeknit)
df['block3'] = df['buildingblock3_smiles'].apply(Toeknit)
...
FEATURES = ['block1', 'block2', 'block3', 'Tok_mol']
...
#X = [ecfp + protein for ecfp, protein, fp in zip(df['ecfp'].tolist(), protein_onehot.tolist(),df['MolSpecial'].tolist())]
#y = df[TARGET].tolist()
